...
if (isset($_GET['aksi'])) {
    ...
    if ($aksi == 'tambah') {
        ...
        if ($row->execute([$nama_supir, $pengalaman, $harga_sewa, $status, $deskripsi, $foto])) {
        ...
        $row_old = $koneksi->prepare($sql_old);
        $row_old->execute([$id_supir]);
        $old_supir = $row_old->fetch(PDO::FETCH_ASSOC);
        $foto = $old_supir['foto'];
        ...
        $row = $koneksi->prepare($sql);
        if ($row->execute([$nama_supir, $pengalaman, $harga_sewa, $status, $deskripsi, $foto, $id_supir])) {
